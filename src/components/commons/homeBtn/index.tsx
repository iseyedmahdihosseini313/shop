...
const HomeBtn = ({ setHomeBtnActive }: IHomeBtn) => {
    const buttonStyle = {
        base: 'flex justify-center items-center p-1 fixed min-w-12 min-h-12 transition-all duration-1000 left-50 -bottom-[1.299rem] active:scale-125 hover:scale-125 ease-in-out bg-black rounded-t-full before:content[""] before:absolute before:size-10 before:rounded-full before:-left-[2.3rem] before:bottom-5  before:content[""] after:absolute after:size-10 after:rounded-full after:content[""] after:left-[2.9rem] after:bottom-5 shadowAfter shadowBefore rounded-full ',
    }
    return (
        <div
            onClick={() => {
